...
- Ao esconder um elemento focável via `aria-hidden`/`tabindex="-1"` controlado por signal (ex.: um botão flutuante que só aparece condicionalmente, como `layout/scroll-to-top/`), tire o foco dele (`.blur()`) no exato momento em que ele deixa de ficar visível, **se** ele for `document.activeElement` naquele instante — senão o navegador bloqueia a mudança com "Blocked aria-hidden on an element because its descendant retained focus" (achado real via QA do Johnny, 08/09/2026, no `ScrollToTop`: o próprio clique no botão dispara o scroll que o esconde, deixando o foco preso nele). Um `viewChild` pro elemento real é o jeito mais simples de checar isso a partir do código que atualiza o signal de visibilidade.
- Nunca dependa só do evento nativo `scroll` pra refletir o resultado de um `window.scrollTo()` **programático** (ex.: a animação do `AnchorScrollService`) em navegadores mobile — o evento não dispara de forma confiável nesse caso (achado real via QA do Johnny num dispositivo mobile de verdade, 08/09/2026, no `ScrollToTop`: o botão não sumia depois de clicar nele pra voltar ao topo, mas sumia normal quando o usuário rolava manualmente com o dedo, o que só faz sentido se o listener de `scroll` estiver perdendo os eventos disparados pelo `scrollTo()` da animação). Se um componente precisa reagir ao destino de um scroll disparado por clique (não só à rolagem manual do usuário), resolva o estado direto no próprio handler do clique, sem esperar o listener de `scroll` confirmar depois.
- Não replique a checagem `event.button === 0 && !event.metaKey && !event.ctrlKey && !event.shiftKey && !event.altKey` (do `AnchorScrollService.shouldHandleClick`, pensada pra distinguir clique-de-mouse-simples de "abrir em nova aba" num desktop) em outro handler de clique achando que é segura por padrão — um clique **sintetizado a partir de toque mobile** não necessariamente preenche esses campos do jeito que um clique de mouse real preenche, e se a checagem falhar silenciosamente no toque, qualquer lógica condicionada a ela nunca roda (achado real via QA do Johnny, 08/09/2026: a primeira correção do bug de scroll acima, em `ScrollToTop.onClick()`, replicou essa checagem pra decidir se escondia o botão, e o botão continuou preso visível no mobile por causa disso — a correção final removeu a checagem e passou a esconder sempre, já que não existe cenário real de "abrir em nova aba" pra um botão que só rola a própria página).
- Em qualquer elemento interativo com **visibilidade condicional** (aparece/some via signal, não sempre visível) que também tenha uma regra `&:hover` de opacidade/cor, escope o hover ao estado visível (`&--visible:hover`, não `&:hover` solto) e prefira envolver em `@media (hover: hover)` — uma pseudo-classe `:hover` tem mais especificidade CSS que a regra base de "escondido" (`opacity: 0`), e em toque mobile, sem mouse pra "destravar" o hover depois do toque, o navegador mantém `:hover` ativo indefinidamente (comportamento padrão de touch UI) — a opacidade de hover vence a de escondido **pra sempre**, mesmo com o signal/`aria-hidden`/`tabindex` todos corretos por baixo. Esse foi o causa raiz **real** de um bug no `ScrollToTop` (achado via QA do Johnny num dispositivo mobile real, 08/09/2026) que pareceu, por três rounds seguidos, ser um problema de JS (foco preso, evento `scroll` não confiável, `event.button` no toque) — só apareceu ao inspecionar `getComputedStyle(el).opacity` de verdade no navegador, não só o estado logico (signal/atributos ARIA), que já estava correto desde a primeira tentativa. Ao investigar um "elemento não some" relatado só em mobile, cheque a opacidade computada ANTES de assumir que é um bug de estado/JS.
