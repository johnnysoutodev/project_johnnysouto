...
- [ ] Componentes reutilizáveis (header, seções do CV, footer), já preparados para os dois temas (ex.: via CSS variables/tokens, não cores fixas).
  ...
  - [x] Experience (8.6/8.11) — `angular-app/src/app/features/experience/`, 6 entradas reais da "Experiência Profissional" de `src/pt/index.html` (Telefônica Brasil como 2 itens — SOC e OSS), logo de empresa virou texto (seção 10), sem conector visual de timeline (8.6, confirmado ausente).
  - [x] Work (8.5) — `angular-app/src/app/features/work/`, `host: { id: 'work' }`, 2 projetos reais (Pitcher, ProFuturo — não 3, seção 10), imagem vira bloco neutro, tags de tecnologia extraídas da descrição real de cada projeto; ação do card omitida no Pitcher (sem link real) e apontando para o link real do ProFuturo já existente no legado (Fundação Telefônica Vivo).
