...
- **Skills — ícones sem match**: dos skills reais (Spring Framework/Spring Boot, Node.js, Java SE 8, Servlet/JSP, OOP, MySQL, Oracle Database, SQL Server, HTML5, CSS3, RWD, JavaScript, jQuery, LESS, Grunt, Git, soft skills), só 3 têm ícone já exportado com match direto: `icon-javascript`, `icon-nodejs`, `icon-git` (seção 6). Os demais **não têm ícone** disponível (o Figma tem ícones de uma stack mais moderna — Next.js, Nest, Socket.io, Tailwind, Cypress, Storybook — que não são skills reais do Johnny) — decisão: renderizar esses itens **texto-only**, sem inventar ou baixar ícone novo de outra fonte.
...
- **Logos de empresa (Experience)**: a spec do item de timeline (seção 8.6) prevê um logo de empresa por item (placeholder fictício "logo-upwork" no Figma). As empresas reais do Johnny (Coca-Cola FEMSA, Telefônica Brasil, Telefônica Educação Digital, RCS Sistemas, Nielsen do Brasil) não têm logo exportado do Figma, e não é apropriado usar/gerar marca de terceiros sem fonte oficial — decisão: o slot de logo vira **texto** (nome da empresa) no componente Angular, não uma extração do Figma.
